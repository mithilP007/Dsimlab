...
// ── /api/simulations/* alias routes ─────────────────────────────────────────
// The frontend simulationStore calls /api/simulations/:id, /api/simulations/:id/decisions, etc.
// These are mapped here to the actual backend simulation handlers.

// POST /api/simulations → start simulation (alias for /api/v1/simulation/start)
app.post('/api/simulations', { preHandler: [requireAuth] }, async (req, reply) => {
  ...
  if (!classId) {
    return reply.status(200).send({
      success: false,
      error: 'You must join a class cohort before starting a simulation.',
      message: 'You must join a class cohort before starting a simulation.',
    });
  }
  ...
  if (existingState) {
    return reply.status(200).send({
      ...
      classId: existingState.classId,
      currentRound: existingState.currentRound,
      isCompleted: existingState.isCompleted,
      status: existingState.status,
      cumulativeSpend: existingState.cumulativeSpend ?? 0,
      cumulativeRevenue: existingState.cumulativeRevenue ?? 0,
  ...
  return reply.status(201).send({
    ...
    cumulativeSpend: newState.cumulativeSpend ?? 0,
    cumulativeRevenue: newState.cumulativeRevenue ?? 0,
    score: newState.score ?? 0,
  });
});

// GET /api/simulations/:id → fetch simulation state by ID
app.get('/api/simulations/:id', { preHandler: [requireAuth] }, async (req, reply) => {
  ...
  return reply.status(200).send({
    ...
    status: state.status,
    cumulativeSpend: state.cumulativeSpend ?? 0,
    cumulativeRevenue: state.cumulativeRevenue ?? 0,
    score: state.score ?? 0,
  });
});

// POST /api/simulations/:id/decisions → alias for /api/v1/simulation/advance
app.post('/api/simulations/:id/decisions', { preHandler: [requireAuth] }, async (req, reply) => {
  const { id } = req.params as { id: string };
  const authReq = req as AuthenticatedRequest;
  ...
  // Decisions are saved via campaign/decision; here we just acknowledge
  return reply.status(200).send({ success: true, message: 'Decisions acknowledged.' });
});

// POST /api/simulations/:id/advance → alias for /api/v1/simulation/advance
app.post('/api/simulations/:id/advance', { preHandler: [requireAuth] }, async (req, reply) => {
  const { id } = req.params as { id: string };
  const authReq = req as AuthenticatedRequest;
  ...
  const { processSimulationRound } = await import('./services/simulation/engine');
  const result = await processSimulationRound(state.id);
  return reply.status(200).send({ success: true, result });
});

// GET /api/simulations/:id/metrics → alias for simulation metrics
app.get('/api/simulations/:id/metrics', { preHandler: [requireAuth] }, async (req, reply) => {
  const { id } = req.params as { id: string };

  const progress = await prisma.studentSimulationProgress.findMany({
    where: { simulationId: id },
    orderBy: { currentDay: 'asc' },
  });

  return reply.status(200).send({ success: true, metrics: progress });
});

// GET /api/simulations/:id/snapshots → alias for simulation snapshots
app.get('/api/simulations/:id/snapshots', { preHandler: [requireAuth] }, async (req, reply) => {
  const { id } = req.params as { id: string };

  const progress = await prisma.simulationProgress.findMany({
    where: { simulationId: id },
    orderBy: { roundNumber: 'asc' },
  });

  return reply.status(200).send({ success: true, snapshots: progress });
});
